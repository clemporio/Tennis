...
def test_parse_shadow_table_picks_eod_section_when_bod_section_exists():
    """REGRESSION: real daily reports have TWO shadow tables — BOD (no Outcome
    column) followed by EOD (with Outcome). Parser must skip the BOD section
    and use the EOD one. Hit by 2026-05-12.md and every real report."""
    from tools.backfill_shadow_outcomes import parse_shadow_table

    md = """# Tennis Daily Report — 2026-05-12

## Shadow Picks (tier B, 70-80% — not placed)

| Pick | Opponent | League | Surface | Model Prob | Fair Odds | Match (UTC) |
|---|---|---|---|---:|---:|---|
| Karen Khachanov | Dino Prizmic | ATP Rome | clay | 0.7713 | 1.297 | 2026-05-12 09:00 |

---
_Generated by tennis_identifier.py_
...
| Pick | Opponent | League | Surface | Model Prob | Fair Odds | Match (UTC) | Outcome | Theo PnL |
|---|---|---|---|---:|---:|---|---|---:|
| Karen Khachanov | Dino Prizmic | ATP Rome | clay | 0.7713 | 1.297 | 2026-05-12 09:00 | LOSS | $-25.00 |

"""
    rows = parse_shadow_table(md, date(2026, 5, 12))
    assert len(rows) == 1
    assert rows[0]["pick"] == "Karen Khachanov"
    assert rows[0]["status"] == "LOSS"
    assert rows[0]["theoretical_pnl"] == pytest.approx(-25.0)
